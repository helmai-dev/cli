...
import { DaemonSocket } from "./daemon-socket.js";
import {
  canAcceptMore,
  getActiveRunDetails,
  getStats,
...
function writeDaemonStatus(): void {
  ...
  const status: DaemonStatus = {
    ...
    transport: {
      stream_url: daemonSocket.getUrl(),
      stream_connected: daemonSocket.isConnected(),
...
async function heartbeat(): Promise<PendingRun[]> {
  ...
  try {
    ...
    lastStreamError = daemonSocket.getLastError();
    ...
    backoffMs =
      backoffMs === 0 ? 5_000 : Math.min(backoffMs * 2, MAX_BACKOFF_MS);
    return [];
  }
}
...
async function processScheduledRuns(
  ...
  for (const scheduledRun of scheduledRuns) {
    ...
    try {
      ...
      log(`Scheduled run ${scheduledRun.run_ulid} failed: ${message}`);
    ...
    if (fs.existsSync(pidPath)) {
      ...
      if (!isNaN(existingPid) && existingPid !== process.pid) {
        try {
          ...
          log(
            "Another daemon already running (PID: " +
              existingPid +
              "), exiting",
          );
          process.exit(0);
...
const daemonSocket = new DaemonSocket({
  log,
  async onPendingRuns(pendingRuns): Promise<void> {
    await processPendingRuns(pendingRuns);
    writeDaemonStatus();
  },
  async onRunnerCommands(commands): Promise<string[]> {
    const handled: string[] = [];

    for (const command of commands) {
      const wasHandled = await handleRunnerCommand(
        command,
        log,
        writeDaemonStatus,
      );
