...
program
  ...
  .option(
    "--link <slug>",
    "Link to an existing Admiral project by slug or ULID",
  )
...
program
  .command("serve")
  .description(
    "Start the Helm MCP server over stdio (used by IDE integrations)",
  )
  .action(async () => {
    await serveCommand();
  });
...
program
  ...
  .action(async () => {
    ...
    const { getInstallSource, getUpdateCommandForSource } =
      await import("./lib/config.js");
    const { stopDaemonIfRunning } = await import("./commands/daemon.js");
    ...
    try {
      ...
      if (daemonWasRunning) {
        try {
          ...
          console.log(
            chalk.yellow(
              "  Could not restart daemon. Run: helm daemon start\n",
            ),
          );
        }
      } else {
